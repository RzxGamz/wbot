...
const addSewaGroup = (gid, expired, _dir) => {
    ...
    _dir.push(obj)
    fs.writeFileSync('./databases/sewa.json', JSON.stringify(_dir))
}
    ...
    setInterval(() => {
        ...
        if (position !== null) {
            console.log(`Sewa expired: ${_dir[position].id}`);
                sock.sendMessage(_dir[position].id, `*Waktu sewa di grup ini sudah habis, bot akan keluar otomatis!*\n\nTerimakasih telah sewa bot ini`, MessageType.text)
                .then(() => {
                    sock.groupLeave(_dir[position].id)
